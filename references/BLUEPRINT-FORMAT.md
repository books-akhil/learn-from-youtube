...
2. **Clean the VTT into a timestamped transcript** with the bundled script. The script lives in the skill directory, not the workspace — prefix the skill directory's absolute path:
   `uv run --no-project python <skill-dir>/scripts/clean_vtt.py <video-id>.en.vtt 30 > transcript.txt`
   (`--no-project` is intentional — the script imports only `re` and `sys`, so it needs no workspace environment, and this works from the temporary directory on machines where `python` itself is not on PATH.)
